...
regarding bank security practices, incorporating validated psychological constructs.
"""
...
class BehavioralIntentionDatasetGenerator:
    ...
    def generate_attitude_construct(self):
        """Generate Attitude (ATT) items - 7-point Likert scale"""
        # Base attitude influenced by demographics and other factors
    ...
    def generate_subjective_norm_construct(self):
        """Generate Subjective Norm (SN) items - 7-point Likert scale"""
    ...
    def generate_perceived_behavioral_control_construct(self):
        """Generate Perceived Behavioral Control (PBC) items - 7-point Likert scale"""
    ...
    def generate_intention_construct(self):
        """Generate Intention (INT) items - 7-point Likert scale"""
    ...
    def generate_threat_appraisal_construct(self):
        """Generate Threat Appraisal items - 7-point Likert scale"""
    ...
    def generate_coping_appraisal_construct(self):
        """Generate Coping Appraisal items - 7-point Likert scale"""
    ...
    def generate_past_behavior_construct(self):
        """Generate Past Behavior (Habit) - 7-point Likert scale"""
    ...
    def create_realistic_correlations(self, data_dict):
        """Create realistic correlations between constructs based on TPB and PMT theory"""
        df = pd.DataFrame(data_dict)
        
        # Create composite scores for each construct
        df['ATT_composite'] = df[['ATT1', 'ATT2', 'ATT3']].mean(axis=1)
        df['SN_composite'] = df[['SN1', 'SN2']].mean(axis=1)
        df['PBC_composite'] = df[['PBC1', 'PBC2', 'PBC3']].mean(axis=1)
        df['INT_composite'] = df[['INT1', 'INT2']].mean(axis=1)
        df['Threat_composite'] = df[['Perceived_Severity', 'Perceived_Vulnerability']].mean(axis=1)
        df['Coping_composite'] = df[['Self_Efficacy', 'Response_Efficacy']].mean(axis=1)
        
        # Apply theoretical correlations
        # Attitude -> Intention (strong positive correlation ~0.6)
        df['INT_composite'] = 0.6 * df['ATT_composite'] + 0.4 * df['INT_composite']
        
        # Subjective Norm -> Intention (moderate positive correlation ~0.4)
        df['INT_composite'] = 0.4 * df['SN_composite'] + 0.6 * df['INT_composite']
        
        # PBC -> Intention (moderate positive correlation ~0.5)
        df['INT_composite'] = 0.5 * df['PBC_composite'] + 0.5 * df['INT_composite']
        
        # Threat Appraisal -> Intention (moderate positive correlation ~0.3)
        df['INT_composite'] = 0.3 * df['Threat_composite'] + 0.7 * df['INT_composite']
        
        # Coping Appraisal -> Intention (strong positive correlation ~0.5)
        df['INT_composite'] = 0.5 * df['Coping_composite'] + 0.5 * df['INT_composite']
        
        # Past Behavior -> Intention (strong positive correlation ~0.6)
        df['INT_composite'] = 0.6 * df['Past_Behavior'] + 0.4 * df['INT_composite']
        
        # PBC -> Coping Appraisal (strong positive correlation ~0.7)
        df['Coping_composite'] = 0.7 * df['PBC_composite'] + 0.3 * df['Coping_composite']
        
        # Threat Appraisal -> Attitude (moderate positive correlation ~0.4)
        df['ATT_composite'] = 0.4 * df['Threat_composite'] + 0.6 * df['ATT_composite']
        
        # Recalculate individual items based on composite scores
        for i, col in enumerate(['ATT1', 'ATT2', 'ATT3']):
            df[col] = df['ATT_composite'] + np.random.normal(0, 0.5, len(df))
        
        for i, col in enumerate(['SN1', 'SN2']):
            df[col] = df['SN_composite'] + np.random.normal(0, 0.5, len(df))
        
        for i, col in enumerate(['PBC1', 'PBC2', 'PBC3']):
            df[col] = df['PBC_composite'] + np.random.normal(0, 0.5, len(df))
        
        for i, col in enumerate(['INT1', 'INT2']):
            df[col] = df['INT_composite'] + np.random.normal(0, 0.5, len(df))
        
        for col in ['Perceived_Severity', 'Perceived_Vulnerability']:
            df[col] = df['Threat_composite'] + np.random.normal(0, 0.5, len(df))
        
        for col in ['Self_Efficacy', 'Response_Efficacy']:
            df[col] = df['Coping_composite'] + np.random.normal(0, 0.5, len(df))
        
        # Clip all values to 1-7 range
        likert_cols = ['ATT1', 'ATT2', 'ATT3', 'SN1', 'SN2', 'PBC1', 'PBC2', 'PBC3', 
    ...
    def generate_dataset(self):
        ...
        print("Generating comprehensive behavioral intention dataset...")
        
        # Generate all constructs
        demographics = self.generate_demographics()
        attitude = self.generate_attitude_construct()
        subjective_norm = self.generate_subjective_norm_construct()
        pbc = self.generate_perceived_behavioral_control_construct()
        intention = self.generate_intention_construct()
        threat_appraisal = self.generate_threat_appraisal_construct()
        coping_appraisal = self.generate_coping_appraisal_construct()
        past_behavior = self.generate_past_behavior_construct()
        ...
        df = self.create_realistic_correlations(all_data)
        
        # Remove composite columns
        composite_cols = ['ATT_composite', 'SN_composite', 'PBC_composite', 
                         'INT_composite', 'Threat_composite', 'Coping_composite']
        df = df.drop(columns=composite_cols)
        
        print(f"Dataset generated with {len(df)} participants and {len(df.columns)} variables")
        return df
    ...
    def create_data_dictionary(self):
        ...
        data_dict = {
            "dataset_info": {
                "title": "Behavioral Intention Dataset - Bank Security Practices",
                "description": "Comprehensive dataset for studying behavioral intentions regarding bank security practices using Theory of Planned Behavior (TPB) and Protection Motivation Theory (PMT)",
            ...
            "behavioral_measures": {
                "past_behavior": {
                    "description": "Self-reported frequency of following security steps in past 3 months",
                    "items": {
            ...
            "statistical_properties": {
                "correlations": "Realistic correlations based on TPB and PMT theory",
                "variance": "Adequate variance for statistical analysis",
                "reliability": "Internal consistency designed to be >0.7 for all scales",
...
def main():
    ...
    # Generate dataset
    generator = BehavioralIntentionDatasetGenerator(n_samples=2000)
    ...
    # CSV format
    df.to_csv('/workspace/behavioral_intention_dataset.csv', index=False)
    print("✓ CSV exported: behavioral_intention_dataset.csv")
    
    # Excel format
    with pd.ExcelWriter('/workspace/behavioral_intention_dataset.xlsx', engine='openpyxl') as writer:
    ...
    print("✓ Excel exported: behavioral_intention_dataset.xlsx")
    
    # JSON format
    df.to_json('/workspace/behavioral_intention_dataset.json', orient='records', indent=2)
    print("✓ JSON exported: behavioral_intention_dataset.json")
    
    # Save data dictionary separately
    with open('/workspace/data_dictionary.json', 'w') as f:
        json.dump(data_dict, f, indent=2)
    print("✓ Data dictionary exported: data_dictionary.json")
    ...
    tpb_vars = ['ATT1', 'ATT2', 'ATT3', 'SN1', 'SN2', 'PBC1', 'PBC2', 'PBC3', 'INT1', 'INT2']
    print(df[tpb_vars].corr().round(3))
    
    print(f"\nCorrelation Matrix for PMT Constructs:")
    pmt_vars = ['Perceived_Severity', 'Perceived_Vulnerability', 'Self_Efficacy', 'Response_Efficacy']
    print(df[pmt_vars].corr().round(3))
    
    print("\n" + "=" * 60)
    print("DATASET GENERATION COMPLETE!")
    print("Files created:")
    print("- behavioral_intention_dataset.csv")
    print("- behavioral_intention_dataset.xlsx") 
    print("- behavioral_intention_dataset.json")
